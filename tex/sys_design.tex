...
本文提出基于容器的主动式云资源管理模型，旨在在负载多变的场景下，通过主动式的调整来提升容器云中服务对负载变化的响应能力，加强容器云对负载变化的灵活性，从而更好地应对负载变化，提升容器云整体的服务质量。在该模型中，我们通过提前感知负载变化和降低容器云中服务调整操作的延迟这两个方法来实现主动式的调整。为了实现对负载变化的提前感知，我们在该模型中记录下相应的负载状态，并根据负载的历史记录对接下来的负载变化进行预测；随后，我们根据容器云中节点的资源状态和服务的要求来实现对任务调度选择的优化，从而降低了服务在容器云中进行调整的操作延迟。接下来，我们会先在第\ref{sec:container_base}节中对容器云中相关的基础技术进行介绍，然后在随后的章节中对该主动式容器云资源管理模型的设计进行介绍。

\section{容器云相关技术分析}\label{sec:container_base}
\subsection{Docker镜像}
Docker镜像是Docker容器运行的基础，提供了运行容器所需要的依赖环境和环境配置。Docker镜像本身基于分层文件系统，将所有的文件变化和环境配置变更都作为内部文件系统层级的变更，本质上是一个包含着一系列文件系统层级修改的有序集合\footnote{https://docs.docker.com/glossary/?term=image}。由于Docker容器利用了写入时复制(copy-on-write，CoW)作为容器内部分层式文件系统读写策略，基于相同Docker镜像创建的不同容器实例可以共享这个镜像底层的文件系统，这大大方便了Docker镜像的制作。因此，Docker用户常常会引用其他基础镜像作为自身镜像的基础。
...
\section{本章小结}
本章从系统设计的角度对本文提出的基于容器的主动式容器云资源管理模型进行了详细介绍。我们在本章中将模型的构成分为三个部分：负载监测和预测、资源供给和管理以及负载应对，介绍了各组成部分的结构，并说明了各部分所包含模块之间的联系。随后我们按照不同组成部分的分类，对其中模块的设计和使用到的和核心算法进行了详细的介绍和分析。我们对当前已有的容器云框架进行了深入的探讨和分析，通过引入服务可用性模型和改善当前调度算法的方式对当前容器云框架存在的一些问题提出了解决方案，并对引入的服务可用性模型和任务调度算法进行了分析和讨论。